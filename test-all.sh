...
grep -s -q "^#include \"ListyFib\.h\"" ListyFib.c
grep_val=$?

if [[ $grep_val != 0 ]]; then
	echo ""
	echo "  Whoa, buddy! Your ListyFib.c file does not appear to have"
	echo "  a proper #include statement for ListyFib.h. Please read"
...
if ! [ -x "$(command -v grep)" ]; then
	...
	num_spc_lines=`grep "^ " ListyFib.c | wc -l | awk '{$1=$1};1'`
	num_tab_lines=`grep "$(printf '^\t')" ListyFib.c | wc -l | awk '{$1=$1};1'`
	num_und_lines=`grep "$(printf '^[^\t ]')" ListyFib.c | wc -l | awk '{$1=$1};1'`
	...
	if [ "$num_spc_lines" -gt 0 ] && [ "$num_tab_lines" -gt 0 ]; then
		...
		echo ""
		echo "     grep \"^ \" ListyFib.c -n"
		echo "     grep \"\$(printf '^\t')\" ListyFib.c -n"
